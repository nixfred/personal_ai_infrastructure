...
- **OAuth billing is for {{PRINCIPAL_NAME}} only.** Never route another human's request through `claude` OAuth/keychain billing. Channels that respond to non-{{PRINCIPAL_NAME}} humans (iMessage with external handles, Telegram bot replies, email auto-reply, SaaS backends, customer agents) MUST set `ANTHROPIC_API_KEY` and NOT delete it. Constitutional rule: see "Personal Use Boundary".
